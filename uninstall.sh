...
if ! command -v jq &>/dev/null; then
    echo "jq is required to uninstall. Please remove the claude-code-now hook entries"
...
# Build a jq filter that removes claude-code-now entries from all events
FILTER=""
...
echo "Uninstalled claude-code-now successfully (6 hooks removed)."
